...
/**
 * \addtogroup csl-hdmi-dongle
 *
 * API for the csl-hdmi-dongle board.
 * @{
 */
...
/** 
 * @brief Voltage settings supported for the xcore core supply. Set by csl-hdmi-dongle_core_voltage_set().
 */
typedef enum {
...
/** @}*/ // END: addtogroup csl-hdmi-dongle


/** @struct csl-hdmi-dongle_config_t
 *  Configuration struct type for setting the hardware profile.
 * 
 *  @var csl-hdmi-dongle_config_t::clk_mode
 *      See csl-hdmi-dongle_mclk_modes_t for available clock mode options.
 *  @var csl-hdmi-dongle_config_t::dac_is_clock_master
 *      Boolean setting for whether the DAC or the xcore.ai is I2S clock master. Set to 0 to make the xcore.ai master.
 *  @var csl-hdmi-dongle_config_t::default_mclk
 *      Nominal clock frequency in MHz. Standard rates are supported between 11.2896 MHz and 49.152 MHz.
 *  @var csl-hdmi-dongle_config_t::pll_sync_freq
 ...
 *  @var csl-hdmi-dongle_config_t::pcm_format
 *      See csl-hdmi-dongle_pcm_format_t for available data frame options.
 *  @var csl-hdmi-dongle_config_t::i2s_n_bits
 *      Number of bits per data frame in I2S.
 *  @var csl-hdmi-dongle_config_t::i2s_chans_per_frame
...
/** 
 * @brief Starts an I2C master task. Must be started from tile[0] *after* csl-hdmi-dongle_board_setup() and *before* and tile[1] HW calls.
 *
 ...
 *  and *before* csl-hdmi-dongle_AudioHwInit() is called.
 *
 *  \param   config     Reference to the csl-hdmi-dongle_config_t configuration struct.
 */
 ...
 *
 *  \param   voltage_setting     See csl-hdmi-dongle_xcore_voltage_t for options
 */
void bsp_core_voltage_set(const bsp_xcore_voltage_t voltage_setting);

/** 
 * @brief Initialises the audio hardware ready for a configuration. Must be called once *after* csl-hdmi-dongle_board_setup().
 *
 *  \param   i2c        Client side of I2C master interface connection.
 *  \param   config     Reference to the csl-hdmi-dongle_config_t hardware configuration struct.
 */
...
/** 
 * @brief Powers down the audio hardware. Call csl-hdmi-dongle_AudioHwShutdown() first to avoid clicks/pops
 * csl-hdmi-dongle_board_setup() and csl-hdmi-dongle_AudioHwInit must be called once 
 * *after* this before attempting to configure the hardware with csl-hdmi-dongle_AudioHwConfig() again.
 * Must be called from tile[0]
 */
 ...
 *  HW config calls from tile[1] will block forever. It is possible to re-start csl-hdmi-dongle_i2c_master()
